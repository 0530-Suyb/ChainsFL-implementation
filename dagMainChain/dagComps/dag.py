...
class DAG(object):
    def __init__(self,active_lst_addr='./dagSS/active_list.json',timespan =-1):
        ...
        self.tips_pool = {}  # 交易尖端池，键为交易名称，值为交易时间戳

    def DAG_add(self,trans):
        """
        向DAG中添加交易到活跃交易池，并根据条件从尖端池中删除已批准的交易。
        ...
        if len(self.tips_pool) > 3:  # 当尖端池中的交易数量大于3时
            for ele in approve_lst:  # 遍历该交易批准的交易列表
                if ele in self.tips_pool:  # 如果被批准的交易在尖端池中
                    del self.tips_pool[ele]  # 从尖端池中删除该交易
...
## The DAG_publish function is responsible for saving the newly active_pool as json file and 
    def DAG_publish(self,trans,tipsMore):
        """
        发布交易，更新活跃交易池和尖端池，并将结果保存到JSON文件。
        确保尖端池中的交易数量大于指定的tipsMore数量。

        :param trans: 要发布的交易对象，需包含name、timestamp和apv_trans属性
        :param tipsMore: 尖端池需要保持的最小交易数量
        """
        self.active_pool[trans.name] = trans.timestamp  # 将交易添加到活跃交易池
        self.tips_pool[trans.name] = trans.timestamp  # 将交易添加到尖端池
        approve_lst = trans.apv_trans  # 获取该交易批准的交易列表
        
        # 如果尖端池中的交易数量足够多，可以删除所有被批准的交易
        if len(self.tips_pool) >= (len(approve_lst)+tipsMore):
            for ele in approve_lst:  # 遍历该交易批准的交易列表
                if ele in self.tips_pool:  # 如果被批准的交易在尖端池中
                    del self.tips_pool[ele]  # 从尖端池中删除该交易
        # 如果尖端池中的交易数量大于tipsMore但不足以删除所有被批准的交易
            ...
            while tipsDelNum != 0:  # 循环删除直到达到需要删除的数量
                ...
                if tmpDelTip in self.tips_pool:  # 如果该交易在尖端池中
                    del self.tips_pool[tmpDelTip]  # 从尖端池中删除该交易
    ...
    def DAG_genesisDel(self):
        """
        如果尖端池中存在创世区块，则将其从尖端池中删除，并更新尖端池的JSON文件。
        """
        if 'GenesisBlock' in self.tips_pool.keys():  # 检查尖端池中是否存在创世区块
            del self.tips_pool['GenesisBlock']  # 从尖端池中删除创世区块
...
## !!! Note: This part is important for the realization of product logic.
    def DAG_choose(self,max_num):
        ...
        count = 0  # 已选择的交易数量
        trans_lst = []  # 选中的交易列表
        for trans in self.tips_pool:  # 遍历尖端池中的交易
